...
export const ProjectDetailsPage = () => {
    ...
    const fetchProjectData = async () => {
        try {
            ...
            const tasksResponse = await taskService.getTasks({ pageSize: 100, categoryId: undefined }); // We need projectId in filters
            ...
            const tasksWithAssignments = await Promise.all(
                projectTasks.map(async (task) => {
                    try {
                        const assignments = await taskAssignmentService.getAssignments(task.id);
                        return { ...task, assignments };
                    } catch (err) {
                        console.error(`Failed to load assignments for task ${task.id}:`, err);
                        return task;
                    }
                })
            );
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-8 relative overflow-hidden">
                        ...
                        <div className="flex-1">
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <span className="text-sm text-gray-500 font-medium flex items-center gap-1">
                                    <Clock size={14} /> Created {new Date(project.createdAt).toLocaleDateString()}
                                </span>
                            </div>

                            <h1 className="text-3xl sm:text-4xl font-display font-bold text-gray-900 mb-4 tracking-tight">
                                {project.title}
                            </h1>
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                                <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                                    <div className="text-gray-500 text-xs font-semibold uppercase mb-1">Total Tasks</div>
                                    <div className="text-2xl font-bold text-gray-900">{tasks.length}</div>
                                </div>
                                <div className="bg-green-50 rounded-xl p-3 border border-green-100">
                                    <div className="text-green-600 text-xs font-semibold uppercase mb-1">Completed</div>
                                    <div className="text-2xl font-bold text-green-700">{tasks.filter(t => t.isCompleted).length}</div>
                                </div>
                                <div className="bg-blue-50 rounded-xl p-3 border border-blue-100">
                                    <div className="text-blue-600 text-xs font-semibold uppercase mb-1">In Progress</div>
                                    <div className="text-2xl font-bold text-blue-700">{tasks.filter(t => !t.isCompleted).length}</div>
                                </div>
                                <div className="bg-purple-50 rounded-xl p-3 border border-purple-100">
                                    <div className="text-purple-600 text-xs font-semibold uppercase mb-1">Team</div>
                                    <div className="text-2xl font-bold text-purple-700">{members.length}</div>
                                </div>
                            </div>
                        ...
                        <div className="flex flex-col justify-between items-end gap-6 min-w-[280px]">
                            <div className="flex gap-3 w-full justify-end">
                                <button
                                    ...
                                    className="btn btn-primary shadow-lg shadow-indigo-200 hover:shadow-indigo-300 transition-all flex items-center gap-2 px-6"
                                >
                                    <Plus size={20} />
                                    New Task
                                </button>
                    ...
                    <div className="flex items-center gap-8 mt-10 border-b border-gray-100">
                        <button
                            ...
                            className={`pb-4 text-sm font-medium transition-all relative px-1 ${activeTab === 'tasks'
                                ? 'text-indigo-600'
                        ...
                        <button
                            ...
                            className={`pb-4 text-sm font-medium transition-all relative px-1 ${activeTab === 'members'
                                ? 'text-indigo-600'
                                : 'text-gray-500 hover:text-gray-700'
                                }`}
                        >
                    ...
                    <div className="space-y-4">
                        {tasks.length === 0 ? (
                            <div className="text-center py-12 bg-white rounded-xl border border-gray-100 border-dashed">
                                ...
                                <p className="text-gray-500 mb-6">Get started by creating a task for this project</p>
                            ...
                            tasks.map(task => (
                                <div key={task.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-all flex items-center justify-between group">
                                    <div className="flex items-start gap-4">
                                        <button
                                            ...
                                            className={`mt-1 w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${task.isCompleted ? 'bg-green-500 border-green-500' : 'border-gray-300 hover:border-indigo-500'
                                                }`}
                                        >
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-3 mt-2">
                                                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${task.priority === 'High' || task.priority === 'Critical' ? 'bg-red-100 text-red-700' :
                                                    ...
                                                    <div className="flex items-center gap-1">
                                                        <div className="flex -space-x-2">
                                                            {task.assignments.slice(0, 3).map((assignment) => (
                                                                <div
                                                                    key={assignment.id}
                                                                    className="w-6 h-6 bg-indigo-100 text-indigo-600 rounded-full flex items-center justify-center font-bold text-xs border-2 border-white"
                                    ...
                                    <div className="relative">
                                        <button
                                            ...
                                            className="opacity-0 group-hover:opacity-100 p-2 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all"
                                        >
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {members.map(member => (
                                    <tr key={member.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium ${member.role === 'Owner' ? 'bg-purple-100 text-purple-700' :
...
const AddMemberModal = ({ projectId, onClose, onSuccess }: AddMemberModalProps) => {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 animate-fadeIn">
            <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6 animate-scaleUp">
                ...
                <div className="space-y-4">
                    ...
                    {loading && <div className="text-center py-2 text-sm text-gray-500">Searching...</div>}
                        ...
                        <div className="bg-white border boundary-gray-200 rounded-lg max-h-48 overflow-y-auto shadow-lg">
                            {results.map(user => (
                                ...
                                >
                                    ...
                                    <div>
                                        <div className="font-medium text-sm text-gray-900">{user.displayName}</div>
                    ...
                    {selectedUser && (
                        <div className="bg-indigo-50 p-3 rounded-lg flex items-center gap-3 border border-indigo-100">
                            ...
                            <div>
                                <div className="font-semibold text-indigo-900">{selectedUser.displayName}</div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {['Viewer', 'Editor', 'Owner'].map((r) => (
                                <button
                                    key={r}
                                    onClick={() => setRole(r)}
                                    className={`px-4 py-2 rounded-lg text-sm font-medium border transition-all ${role === r
